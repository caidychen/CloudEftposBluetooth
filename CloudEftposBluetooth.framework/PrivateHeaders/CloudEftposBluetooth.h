...
#define CloudEftposSDK_version 2.0.0
...
@interface CloudEftposBluetooth: NSObject
...
@property (nonatomic, strong) NSString *updateMessage; // Update message
@property (nonatomic) int updatePriority; // Update priority 1, 2, 3
@property (nonatomic) bool updateEnforced; // If the update is enforced
...
@property (nonatomic, strong) UIColor* themePrimaryColor; // The theme color for the transaction screens. (Will be overriden by transactionRequest theme color if neccessary)
 ...
 *
 *  @discussion Static method to get the CloudEftposBluetoothsingleton.
 ...
 */
+(CloudEftposBluetooth*) sharedInstance;

/*!
 *  @method close;
 *
 *  @discussion Static method to delete the CloudEftposBluetoothsingleton.
 ...
 *  @method init:
 *
 *  @discussion Initialises the CloudEftposBluetoothobject.
 */
-(CloudEftposBluetooth*) init;
-(CloudEftposBluetooth*) initWithInstanceIdentifier:(NSString *)identifier;
 ...
 *  will finish with an appropriate result code (to indicate cancellation).
 ...
 *  authorised to interact with the CloudEftposBluetoothsystem.
 */
