 ...
 * 
 * Step 3:
 * Replace your NSLog statements with QPSDDLog statements according to the severity of the message.
 * 
 * NSLog(@"Fatal error, no dohickey found!"); -> QPSDDLogError(@"Fatal error, no dohickey found!");
 * 
 * QPSDDLog works exactly the same as NSLog.
 * This means you can pass it multiple variables just like NSLog.
**/


@class QPSDDLogMessage;

@protocol QPSDDLogger;
@protocol QPSDDLogFormatter;
 ...
 * 
 * The QPSDDLogMessage object includes a 'tag' ivar that may be used for a variety of purposes.
 ...
 * as the QPSDDLogMessage class captures the individual flag that caused the log message to fire.
 * 
 * -- Levels --
 * 
 ...
 *         QPSDDLogVerbose(@"sprocket: %@", [sprocket description])
 * }
 * 
 * -- Async --
 * 
...
#define QPSDDLogError(frmt, ...)   LOG_OBJC_MAYBE(LOG_ASYNC_ERROR,   ddLogLevel, LOG_FLAG_ERROR,   0, frmt, ##__VA_ARGS__)
#define QPSDDLogWarn(frmt, ...)    LOG_OBJC_MAYBE(LOG_ASYNC_WARN,    ddLogLevel, LOG_FLAG_WARN,    0, frmt, ##__VA_ARGS__)
#define QPSDDLogInfo(frmt, ...)    LOG_OBJC_MAYBE(LOG_ASYNC_INFO,    ddLogLevel, LOG_FLAG_INFO,    0, frmt, ##__VA_ARGS__)
#define QPSDDLogVerbose(frmt, ...) LOG_OBJC_MAYBE(LOG_ASYNC_VERBOSE, ddLogLevel, LOG_FLAG_VERBOSE, 0, frmt, ##__VA_ARGS__)

#define QPSDDLogCError(frmt, ...)   LOG_C_MAYBE(LOG_ASYNC_ERROR,   ddLogLevel, LOG_FLAG_ERROR,   0, frmt, ##__VA_ARGS__)
#define QPSDDLogCWarn(frmt, ...)    LOG_C_MAYBE(LOG_ASYNC_WARN,    ddLogLevel, LOG_FLAG_WARN,    0, frmt, ##__VA_ARGS__)
#define QPSDDLogCInfo(frmt, ...)    LOG_C_MAYBE(LOG_ASYNC_INFO,    ddLogLevel, LOG_FLAG_INFO,    0, frmt, ##__VA_ARGS__)
#define QPSDDLogCVerbose(frmt, ...) LOG_C_MAYBE(LOG_ASYNC_VERBOSE, ddLogLevel, LOG_FLAG_VERBOSE, 0, frmt, ##__VA_ARGS__)

/**
 * The THIS_FILE macro gives you an NSString of the file name.
 * For simplicity and clarity, the file name does not include the full path or file extension.
 * 
 * For example: QPSDDLogWarn(@"%@: Unable to find thingy", THIS_FILE) -> @"MyViewController: Unable to find thingy"
**/

NSString *QPSDDExtractFileNameWithoutExtension(const char *filePath, BOOL copy);

#define THIS_FILE (QPSDDExtractFileNameWithoutExtension(__FILE__, NO))

/**
 * The THIS_METHOD macro gives you the name of the current objective-c method.
 * 
 * For example: QPSDDLogWarn(@"%@ - Requires non-nil strings", THIS_METHOD) -> @"setMake:model: requires non-nil strings"
 * 
...
+ (void)addLogger:(id <QPSDDLogger>)logger;
+ (void)removeLogger:(id <QPSDDLogger>)logger;
...
@protocol QPSDDLogger <NSObject>
@required

- (void)logMessage:(QPSDDLogMessage *)logMessage;
...
**/
- (id <QPSDDLogFormatter>)logFormatter;
- (void)setLogFormatter:(id <QPSDDLogFormatter>)formatter;
 ...
 * 
 * This allows invocations of QPSDDLog's flushLog method to be propogated to loggers that need it.
 * 
 * Note that QPSDDLog's flushLog method is invoked automatically when the application quits,
...
@protocol QPSDDLogFormatter <NSObject>
@required
...
**/
- (NSString *)formatLogMessage:(QPSDDLogMessage *)logMessage;
...
**/
- (void)didAddToLogger:(id <QPSDDLogger>)logger;
- (void)willRemoveFromLogger:(id <QPSDDLogger>)logger;
...
@protocol QPSDDRegisteredDynamicLogging
...
/**
 * The QPSDDLogMessage class encapsulates information about the log message.
 * If you write custom loggers or formatters, you will be dealing with objects of this class.
**/

enum {
	QPSDDLogMessageCopyFile     = 1 << 0,
	QPSDDLogMessageCopyFunction = 1 << 1
};
typedef int QPSDDLogMessageOptions;

@interface QPSDDLogMessage : NSObject
{
...
@public
	...
	// For 3rd party extensions that manually create QPSDDLogMessage instances.
	QPSDDLogMessageOptions options;
}
 ...
 * Options is a bitmask which supports QPSDDLogMessageCopyFile and QPSDDLogMessageCopyFunction.
**/
- (id)initWithLogMsg:(NSString *)logMsg
            ...
            function:(const char *)function
                line:(int)line
                 tag:(id)tag
             options:(QPSDDLogMessageOptions)optionsMask;
...
/**
 * The QPSDDLogger protocol specifies that an optional formatter can be added to a logger.
...
@interface QPSDDAbstractLogger : NSObject <QPSDDLogger>
{
	id <QPSDDLogFormatter> formatter;
